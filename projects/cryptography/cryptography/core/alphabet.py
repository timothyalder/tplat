...
class Alphabet:
    ...
    def __new__(
        cls,
        alphabet: Union["Alphabet", List[str], List[Tuple[str, float]], None] = None,
    ):
    ...
    def __init__(
        ...
        self.canonical_alphabet, self.canonical_probabilities = map(
            list,
            zip(
                *[
                    ("A", 8.12),
        ...
        if alphabet is None:
            ...
            canonical_index = [
                self.canonical_index.get(plaintext, -1) for plaintext in alphabet
            ]
            self.probabilities = [
                self.canonical_probabilities[idx] / 100 for idx in canonical_index
            ]
    ...
    def rotate(self, i: int):
        for attr in ["alphabet", "probabilities"]:
            ...
            shifted.rotate(
                -i
            )  # Although confusing, negate the index to match typical cryptography syntax
            self.__setattr__(attr, list(shifted))

    def calculate_distribution(self, m: str) -> None:
        self.probabilities = [
            m.upper().count(plaintext) / len(m) for plaintext in self.alphabet
        ]
        self.frequency_sort()
    ...
    def frequency_sort(self) -> None:
        self.alphabet, self.probabilities = map(
            ...
            zip(
                *sorted(
                    zip(self.alphabet, self.probabilities), key=lambda x: (-x[1], x[0])
                )
            ),
        )
    ...
    def estimate_i_from_pdf(
        ...
        assert self.__len__() == len(
            c
        ), "Error! Mismatch in reference and ciphertext distribution lengths."
        losses = []
        for _ in range(self.__len__()):
            c.rotate(1)
            loss = sum(
                [
                    abs(plaintext_prob - ciphertext_prob)
                    for plaintext_prob, ciphertext_prob in zip(
                        self.probabilities, c.probabilities
                    )
                ]
            )
            losses.append(loss)
    ...
    def plot_distributions(
        self, alphabets: Union["Alphabet", List["Alphabet"]]
    ) -> Figure:
        ...
        for idx, alphabet in enumerate(alphabets):
            ax.plot(
                range(len(alphabet.alphabet)),
                alphabet.probabilities,
                label=f"{alphabet.__class__.__name__} ({idx})",
            )
        ax.set_xticks(range(len(self.alphabet)))
        ax.set_xticklabels(self.alphabet)
        ax.set_xlabel("Alphabet")
